...
(function(global, factory) {
  ...
  function uriTemplatePart(subject, spec) {
    ...
    vars.forEach(function(varSpec, index) {
      ...
      if (couldBeType('object')) {
        if (!modFunction) modFunction = function(x) {
          return x
        };
  ...
  var Generator = schema2js.Generator = function Generator(config) {
    ...
    this.classVars = {
      GeneratedClass: true
    }; // it's our default superclass, so make sure it won't be used later
    this.aliases = {};
    this.missingMap = {};
  ...
  Generator.prototype = {
    ...
    getFullSchema: function(schema, haltUrls) {
      ...
      if (haltUrls[refUrl]) return {
        "description": "ERROR! Recursion"
      };
      ...
      if (!schema) {
        ...
        return {
          "description": "Missing schema: " + refUrl,
          placeholder: true
        };
      }
    ...
    codeForUrl: function(url, requireUrl) {
      ...
      if (!schema) {
        ...
        schema = {
          "description": "Missing schema: " + url
        };
  ...
  var errorRequestFunction = function(params) {
    throw new Error('Requests not enabled - try JsonModel.setRequestFunction(func):\n' + JSON.stringify(params));
  };
  ...
  function RootModel(dataStore, storeKey) {
    ...
    this.modelForPath = function(path) {
      ...
      for (var i = 0; i < pathParts.length; i++) {
        var key = pathParts[i];
        target = target.c[key] = target.c[key] || {
          c: {}
        };
      }
    ...
    this.getPathErrors = function(path, includeSchemaErrors, immediateOnly) {
      ...
      var result = errors.filter(function(error) {
        return error.path == path || (!immediateOnly && error.path.substring(0, path.length) == path && error.path.charAt(path.length) == '/');
      });
      if (includeSchemaErrors) {
        for (var dataPath in missingSchemas) {
          if (dataPath == path || (!immediateOnly && dataPath.substring(0, path.length) == path && dataPath.charAt(path.length) == '/')) {
            missingSchemas[dataPath].forEach(function(schemaUrl) {
              ...
              if (requestErrors[baseUrl]) {
                result.push({
                  ...
                  path: dataPath,
                  params: {
                    message: requestErrors[baseUrl].message,
                    status: (requestErrors[baseUrl].httpStatus || null)
                  },
                  schema: schemaUrl
                });
              } else {
  ...
  DataStore.prototype = {
    normParams: function(params) {
      if (typeof params === 'string') return this.normParams({
        url: params
      });
      return {
  ...
  function executeDiffDom(subject, target, diff, context, callback) {
    if (target.nodeType === 1) {
      ...
      for (var i = 0; i < target.attributes.length; i++) {
        var key = target.attributes[i].name,
  ...
  function diffDom(subject, target, cullSize, ignoreFirstBinding) {
    ...
    var attributesTotal = 1,
      attributesCorrect = 1;
    for (var i = 0; i < subject.attributes.length; i++) {
      attributesTotal++;
      var key = subject.attributes[i].name,
        value = subject.attributes[i].value;
    ...
    var diagonal = 1,
      endDiagonal = subjectCount + targetCount + 1;
    while (diagonal < endDiagonal) {
      ...
      for (var subjectIndex = Math.max(0, diagonal - targetCount); subjectIndex <= subjectCount && subjectIndex <= diagonal; subjectIndex++) {
        ...
        if (addFrom) {
          ...
          if (score > bestScore) {
            best = {
              score: addFrom.score + score,
              path: addFrom.path.concat([subjectIndex]),
              actions: addFrom.actions.concat(['add ' + targetNode])
            };
          }
        }
        ...
        if (removeFrom) {
          ...
          if (score > bestScore) {
            best = {
              score: removeFrom.score + score,
              path: removeFrom.path.concat([subjectIndex]),
              actions: removeFrom.actions.concat(['remove' + subjectNode])
            };
          }
        }
        var mergeFrom = prevOptions[subjectIndex - 1];
        if (mergeFrom) {
          var diff = diffDom(subjectNode, targetNode, cullSize) || {
            score: -Infinity
          };
          if (diff.score > bestScore) {
            best = {
              score: mergeFrom.score + diff.score,
              path: mergeFrom.path.concat([null, subjectIndex]),
              actions: mergeFrom.actions.concat(['merge ' + subjectNode + ' -> ' + targetNode, diff])
            };
          }
        }
  ...
  if (typeof require === 'function' && typeof module !== 'undefined') {
    ...
    Bindings.prototype.includeCss = function(filename) {
      var code = require('fs').readFileSync(filename, {
        encoding: 'utf-8'
      });
    ...
    api.bundleJs = function(bindings, includeCss) {
      ...
      var code = ['/json-model.js'].map(function(filename) {
        var result = '/*** ' + filename + ' ***/\n\n';
        result += require('fs').readFileSync(__dirname + filename, {
          encoding: 'utf-8'
        });
        return result;
      });
  ...
  function BindingContext(bindings, dataStore, initialUi) {
    ...
    this.urlForState = function(resourceUrl, newUiState) {
      ...
      return api.util.url.encodeQuery({
        json: resourceUrl
      }) || '?';
    };
